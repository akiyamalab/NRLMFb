...
class NRLMFb:
    ...
    def __init__(self, cfix=5, K1=5, K2=5, num_factors=10, theta=1.0, lambda_d=0.625, lambda_t=0.625, alpha=0.1, beta=0.1, eta1=7, eta2=3, max_iter=100):
        ...
        for d, t in test_data:
            if d in self.train_drugs:
                ...
                if t in self.train_targets:
                    ...
                    val = np.sum(v1*v2)
            score = np.exp(val)/(1+np.exp(val))
            scores.append(self.rescore(score,d,t))
    ...
    def rescore(self,score,d,t):
        Y = self.intMat / self.cfix
        Yd = np.sum(Y,axis=1)
        Yt = np.sum(Y,axis=0)
        gamma = Yd[d]+Yt[t]-2 if Y[d,t]==1 else Yd[d]+Yt[t]
        score_ = (score*(gamma*self.eta1+self.eta2-2)+1)/(gamma*self.eta1+self.eta2)
        return score_

    def __str__(self):
        return "Model:NRLMFb, c:%s, K1:%s, K2:%s, r:%s, lambda_d:%s, lambda_t:%s, alpha:%s, beta:%s, eta1:%s, eta2:%s, theta:%s, max_iter:%s" % (self.cfix, self.K1, self.K2, self.num_factors, self.lambda_d, self.lambda_t, self.alpha, self.beta, self.eta1, self.eta2, self.theta, self.max_iter)
