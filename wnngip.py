# This script was modified by Tomohiro Ban (January 9, 2018)
...
class WNNGIP:
    ...
    def rls_kron_train(self, R, Kd, Kt):
        ...
        y = y.T.reshape((1, y.size)) # original was y = y.reshape((1, y.size))
        z = (x*y).reshape((m, n)).T  # original was z = (x*y).reshape((n, m))
    ...
    def __str__(self):
        return "Model: WNNGIP, T:%s, sigma:%s, alpha:%s, gamma:%s" % (self.T, self.sigma, self.alpha, self.gamma)
